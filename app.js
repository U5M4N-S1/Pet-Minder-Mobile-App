...
function goToAuth() { show('auth'); currentScreen = 'auth'; }
...
// ===== NAVIGATION =====
function goToHome() { show('home'); currentScreen = 'home'; setNavActive('home'); refreshPetsUI(); }
function switchTab(tab) { show(tab); currentScreen = tab; setNavActive(tab); }
...
document.getElementById('chat-input-field').addEventListener('keydown', e => { if (e.key === 'Enter') sendMessage(); });
...
function logout() {
  isAdmin = false;
  show('landing'); currentScreen = 'landing';
  document.getElementById('app-nav').classList.add('hidden');
  document.getElementById('mobile-header').style.display = 'none';
}
